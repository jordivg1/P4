...
if [[ $# < 1 ]]; then
   ...
   echo "           - Now compute_lpcc() can be used."
   echo "           - And also compute_mfcc() can be used."
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           gmm_train  -i 1 -v 1 -t 0.0001 -n1000 -m 72 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       echo gmm_classify -d $w/$FEAT -e $FEAT -D $w/gmm/$FEAT -E gmm $lists/gmm.list  $lists/class/all.test
	 ...
	 # \DONE
	gmm_train  -i 1 -v 1 -t 0.001 -n 1000 -m 72 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/world.gmm $lists/verif/users_and_others.train || exit 1
      ...
      # gmm_train -d $w/mfcc -e mfcc -g $w/gmm/mfcc/world.gmm -m 9 -N 20 -T 0.01 -i 0 -n 20 || exit 1
	   ...
	   # recognized is lists/final/class.test
        for filename in $(cat $lists/final/class.test); do
            ...
            EXEC="wav2lpcc 16 20 spk_ima/sr_test/$filename.wav $w/$FEAT/final/$filename.$FEAT"
            echo $EXEC && $EXEC || exit 1
        done        
...
else {print "0\n"}' $w/final/verif_test.log > $w/final/verif_total.log
