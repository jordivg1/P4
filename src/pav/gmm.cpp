...
#define CTTE_GAUSSIAN -0.9189385332046727 /* log(1 / sqrt(2 * PI)) */
  ...
  */
  ...
  int GMM::em(const fmatrix &data, unsigned int max_it, float inc_threshold, int verbose) {
    ...
    float old_prob=-1e34, new_prob, inc_prob;
    ...
    for (iteration=0; iteration<max_it; ++iteration) {
      ...
      /// \HECHO (falta comprobar)
      new_prob = em_expectation(data, weights); ///Paso Expectation
      em_maximization(data, weights); ///Paso maximization
      inc_prob = new_prob - old_prob; ///Calculamos incremento
      old_prob = new_prob; ///Actualizamos el valor en cada iteración

      if(inc_prob<=inc_threshold) return 0; ///Incremento es más bajo del umbral permitido
